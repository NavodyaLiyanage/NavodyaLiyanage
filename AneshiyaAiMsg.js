...
let { MessageType, MessageOptions, Mimetype, GroupSettingChange, ChatModification } = require('@navodyaliyanage/queen-aneshiya-web');
let fs = require('fs');
let os = require('os');
...
QueenAneshiya['IntroduceCMD']({on: 'text', fromMe: false, dontAdCommandList: true, delownsewcmd: false}, (async (message, input) => {
    if ((message.message.startsWith('Aneshiya') || message.message.startsWith('.bot')) && Details.ANESHIYA_AI !== 'true') {    
        var unique_ident = message.client.user.jid.split('@')[0]      
        var finm = message.message.replace('Aneshiya', '').replace(' ', '').replace('.bot', '')   
        var trmsg = ''
...
QueenAneshiya['IntroduceCMD']({on: 'text', fromMe: false, delownsewcmd: false}, (async (message, input) => {
if(message.message.startsWith(STP)) return;
        if (Details.ANESHIYA_AI == 'true') {
...
QueenAneshiya['IntroduceCMD']({ pattern: 'tellbot$', desc: 'Toke With Artificial Intelligence Bot',dontAdCommandList: true, fromMe: false }, (async (message, input) => {
